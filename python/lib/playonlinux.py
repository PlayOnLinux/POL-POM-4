...
def winpath(script, path):
    ...
    if(arch == ""):
        arch="x86"
    if(ver == ""):
        return(os.popen("env WINEPREFIX='"+os.environ["POL_USER_ROOT"]+"/wineprefix/"+pref.encode("utf-8","replace")+"/' 'wine' winepath -w '"+path.encode("utf-8","replace")+"'").read().encode("utf-8","replace").replace("\n","").replace("\r",""))
    else:
        return(os.popen("env WINEPREFIX='"+os.environ["POL_USER_ROOT"]+"/wineprefix/"+pref.encode("utf-8","replace")+"/' '"+os.environ["POL_USER_ROOT"]+"/wine/"+Variables.os_name+"-"+arch+"/"+ver+"/bin/wine' winepath -w '"+path.encode("utf-8","replace")+"'").read().encode("utf-8","replace").replace("\n","").replace("\r",""))
