...
class SetupWindowHeader(wx.Panel):
    def __init__(self, panel, topImage, leftImage):
...
class SetupWindowFooter(wx.Panel):
    def __init__(self, panel, isProtected):
        self.uiHelper = UIHelper()
        wx.Panel.__init__(parent, -1, size=(522,45), pos=(-1,358), style = self.uiHelper.widgetBorders())
        self.cancelButton = wx.Button(self, wx.ID_CANCEL, _("Cancel"), pos=(425,0),size=(85,self.uiHelper.getSetupWindowButtonHeight()))
        self.nextButton = wx.Button(self.footer, wx.ID_FORWARD, _("Next"), pos=(335,0),size=(85,self.uiHelper.getSetupWindowButtonHeight()))
        
        self.cancelButton.Enable(isProtected)
