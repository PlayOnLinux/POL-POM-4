...
class Controller(object):
   ...
   def eventGuiServer(self, event):
       ...
       elif(command == "POL_Die"):
           playOnLinuxApppolDie()
           self.closeConnexion(scriptPid)    
       
       elif(command == "POL_Restart"):
           playOnLinuxApp.polRestart()
           self.closeConnexion(scriptPid)  
       
       elif(command == 'POL_SetupWindow_Init'):
          if(len(data) == 6):
       ...
       elif(command == 'POL_SetupWindow_Close'):
           try:
           ...
           try:
               ...
               argumentsWithObject = [setupWindowObject] + arguments
               ...
               if(command == 'POL_SetupWindow_Update'):
                  if(setupWindowObject.getCurrentStep() != None):
                      try:
                          setupWindowObject.getCurrentStep().update(*arguments)
                      except AttributeError:
                          Message ( 'Update method does not exist!' )
               else:
                  try:
                      stepClass = getattr(Step, command)
                  except AttributeError:
                      Message ('Class not found "%s" (%s)' % (command, argumentsWithObject) )
                  else:
                      try:
                          setupWindowObject.setCurrentStep(stepClass(*argumentsWithObject))              
                      except TypeError, e:
                          print 'Error: %s (%s)' % (e, argumentsWithObject)
                   
       
   
   def startDownloadFromSetupWindowStep(self, stepClass):
       # Download parameters
       url = stepClass.getUrl()
       localDirectory = stepClass.getLocalDirectory()
       ...
       distantFile.register(stepClass.getDownloadGauge())
       distantFile.register(stepClass.getDownloadText())      
   ...
   def runProgram(self, selectedProgram):
       ...
       try:
           ...
           Message(_("The virtual drive associated with {0} does not exists.").format(_shortcutName))
